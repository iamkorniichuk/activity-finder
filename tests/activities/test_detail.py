class TestDetailActivities:
    ...
    def test_successful_get_recurring(self, client, create_recurring_activity):
    ...
    def test_successful_patch_recurring(self, auth_client, create_recurring_activity):
        url = self.build_url(create_recurring_activity.json()["pk"])
    ...
    def test_type_patch(self, auth_client, create_recurring_activity):
        url = self.build_url(create_recurring_activity.json()["pk"])
        data = {"name": "Super VR game", "type": "OneTimeActivity"}
        response = auth_client.patch(url, data=data)
        assert response.status_code == 400

    def test_nonexistent_field_patch(self, auth_client, create_recurring_activity):
        url = self.build_url(create_recurring_activity.json()["pk"])
